import fitz, io, pytesseract, re, cv2
...
def find_first_page_with_page_number_previous(found_numbers, current_index):
    for num in found_numbers[current_index]:
        if check_for_number(found_numbers[current_index - 1], num - 1) and check_for_number(found_numbers[current_index - 2], num - 2):
            return (current_index - 2, num - 2)
    return (-1, None)
...
for page_index in range(len(doc)):
    ...
    print(f"Processing page {pdf_page_number}")
    page = doc[page_index]
    ...
    if len(parsed_numbers) == 0:
        ...
        if page_index >= 2:
            if first_index_with_page_number == -1:
                ...
                if first_index_with_page_number != -1:
                    print(f"The first page with a page number is {first_index_with_page_number + 1}")
                else:
                    pass
                    # check for expected number
            else:
                if actual_page_number not in parsed_numbers:
                    print(f"WARNING: Expected to find") # TODO
