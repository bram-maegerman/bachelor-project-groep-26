...
class GUI:
            ...
            if os.path.exists(pathname):
                ...
                for file in projects:
                    try:
                        ...
                        print(f"Removed file '{file}_LOG.txt'")

                        # Get the file name (without path) and add 'compressed_' prefix
                        compressed_file = os.path.join(os.path.dirname(file), 'compressed_' + os.path.basename(file))
                        os.remove(compressed_file)
                        print(f"Removed file '{compressed_file}'")

                        # Try removing the date-directory it is in
                        # Example: <project>/<date>/<file>
                        date_directory = os.path.abspath(os.path.dirname(file))
                        print(f"Checking if date directory '{date_directory}' is empty...")

                        if os.path.exists(date_directory) and not os.listdir(date_directory):
                            os.rmdir(date_directory)
                            print(f"Removed empty directory '{date_directory}'")

                        # Remove the project directory if it is empty
                        project_directory = os.path.abspath(os.path.dirname(date_directory))
                        print(f"Checking if project directory '{project_directory}' is empty...")
                        if os.path.exists(project_directory) and not os.listdir(project_directory):
                            os.rmdir(project_directory)
                            print(f"Removed empty project directory '{project_directory}'")
    ...
    def run_script_on_files(self):
        ...
        for index, file_path in enumerate(self.next_run):
            ...
            log_file_location = main_script(str(file_path), str(export_path))
...
def main_script(path_to_pdf: str, log_files_export_path: str):
    ...
    with Manager() as manager:
        ...
        with Pool(processes=cpu_count()) as pool:
            ...
            while completed < doc_len:
                ...
                update_progress(completed / doc_len * 100)
